...
const generateMockAgents = (): Agent[] => {
  ...
  const agents: Agent[] = [
    ...
    },
    {
      ...
      access: ["mcp"],
      description:
        "Helps users discover relevant data assets across the catalog using natural language queries.",
      createdAt: new Date(
    ...
    },
    {
      ...
      access: ["rest"],
      description:
        "Validates table schemas against defined standards and flags inconsistencies.",
      createdAt: new Date(
    ...
    },
    {
      ...
      access: ["mcp", "rest"],
      description:
        "Monitors data pipeline health and sends alerts on failures or delays.",
      createdAt: new Date(
        baseDate.getTime() - 1 * 24 * 60 * 60 * 1000,
      ).toISOString(),
      status: "draft",
    },
    {
      ...
      access: ["mcp"],
      description:
        "Suggests relevant tags for data assets based on content and usage patterns.",
      createdAt: new Date(
    ...
    },
    {
      ...
      access: ["rest"],
      description:
        "Streamlines data access request workflows by guiding users through approval processes.",
      createdAt: new Date(
        baseDate.getTime() - 3 * 24 * 60 * 60 * 1000,
      ).toISOString(),
      status: "draft",
    },
    {
      ...
      access: ["mcp", "rest"],
      description:
        "Analyzes query costs and recommends optimizations to reduce cloud data warehouse spending.",
      createdAt: new Date(
    ...
    },
    {
      ...
      access: ["mcp"],
      description:
        "Scans datasets for PII and sensitive information, applying appropriate classifications.",
      createdAt: new Date(
        baseDate.getTime() - 5 * 24 * 60 * 60 * 1000,
      ).toISOString(),
      status: "draft",
  ...
  return agents.map((agent) =>
    agent.status === "draft" ? { ...agent, creator: "CL" } : agent,
  );
};
...
function AgentsPageContent() {
  ...
  const filteredAndSortedAgents = useMemo(() => {
    ...
    if (categoryFilter === "drafts") {
      filtered = filtered.filter((agent) => agent.status === "draft");
        ...
        {displayAgents.length > 0 && (
          ...
          >
            ...
            <ToggleButtonGroup
              ...
              onChange={(_, newValue) => {
                if (newValue !== null) {
                  setCategoryFilter(newValue as "all" | "native" | "custom" | "drafts");
                }
              }}
              size="small"
              sx={{
                ...
                <tbody>
                  {filteredAndSortedAgents.map((agent) => (
                    <tr
                      key={agent.id}
                      onClick={() =>
                        router.push(`/app/studio/agents-2/${agent.id}`)
                      }
                        ...
                        >
                          ...
                          <div style={{ minWidth: 0, flex: 1 }}>
                            ...
                            >
                              ...
                              {categoryFilter === "drafts" && agent.status === "draft" && (
                                <Chip
                                  label="Draft"
                                  size="small"
                                  sx={{
