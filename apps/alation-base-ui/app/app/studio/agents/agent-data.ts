...
const createFallbackAgent = (id: string): Agent => ({
  id,
  name: `Agent ${id}`,
  type: "conversational",
  category: "custom",
  creator: "Alation",
  access: ["mcp"],
  description: "Agent details are not yet synced for this item.",
...
export const getAgentById = (id: string): Agent => {
  ...
  return agents.find((agent) => agent.id === id) || createFallbackAgent(id);
};
