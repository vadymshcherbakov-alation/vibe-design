...
**Do not use when**
- The list is longer than ~8 entries → use a [Table](./table.md).
- The tile has no action on click → use the base [Card](../base/card.md) primitive; a non-clickable tile should not look clickable.
- The content is a metric / stat → use [Metric Card](./metric-card.md).
- The target is a labelled form control → use [Form Field](./form-field.md).
...
See [`nav-card-preview.html`](./nav-card-preview.html) for a static visual preview.
