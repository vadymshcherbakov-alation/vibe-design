...
- The message blocks the user until dismissed → use [Dialog](../composite/dialog/usage.md)
- The message is a single word label → use [Chip](./chip/usage.md)
...
- **Destructive actions** — an Alert with `severity="error"` is **feedback**, not an action trigger. A destructive *action* still requires a Destructive [Button](./button/usage.md) + [ConfirmDialog](../composite/dialog/usage.md).
