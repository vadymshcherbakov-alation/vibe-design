...
- **Code:** There is **no shared `<Form>` component** in `@alation/alation-ui`. Production forms compose `<Stack>` + the Form Field pattern ([form-field.md](./form-field/usage.md)) directly at the call site. This reference documents that composition.

## 2. Purpose

A vertical stack of labeled fields for creating or editing an entity. Canonical shape: a `Stack spacing={3}` of [Form Field](./form-field/usage.md) wrappers, each containing a `TextField` (text, multiline, or select) or an `Autocomplete` for user/entity search.
...
**Do not use when**
- You need a single inline field inside a toolbar → use [TextField](../base/text-field/usage.md) directly.
- You need a search box in a page header → use [TextField](../base/text-field/usage.md) with an `InputAdornment` icon.
- You need a confirm/destructive flow → wrap the form inside a [Dialog](./dialog/usage.md) if applicable.

## 4. Contract

### Guarantees
- Every field uses [`FormField`](./form-field/usage.md) — labels render as `Typography variant="subtitle1"`.
...
- Every input uses [`TextField`](../base/text-field/usage.md) or `Autocomplete` — no bare `<input>`, no bare MUI `Select`.
...
**Composed of**
- [Form Field](./form-field/usage.md) — label + helper wrapper
- [TextField](../base/text-field/usage.md) — single-line, multiline, select
