...
**Do not use when**
- The list is 1–5 items of the same type → use a `<Stack>` of [Card (Nav variant)](./card.md) tiles.
- The data is hierarchical / tree-shaped → use a tree component (not yet documented).
- The list is a picker inside a form → use [Autocomplete](./form.md) instead.
...
See [`icon-button.md`](../base/icon-button.md) for the accessibility-name contract.
