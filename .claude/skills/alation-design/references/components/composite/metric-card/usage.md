...
- The surface is a clickable entity tile → use [Nav Card](./nav-card/usage.md).
...
- Never invent a bespoke badge pill — use the [Chip](../base/chip/usage.md) pattern (`<Chip variant="filledLight" color="…" size="xsmall" />`).
...
- When the card contains a destructive action, the destructive Button must follow the [Button](../base/button/usage.md) destructive contract (ConfirmDialog required for irreversible actions).
...
- **Title row** — `subtitle1` title + optional right-aligned [Chip](../base/chip/usage.md) (e.g. `<Chip label="This quarter" variant="outlined" size="xsmall" />`).
