...
Where *(one base component)* comes from the list in §6 (Input types): [Text Field](../base/text-field/usage.md) (the most common — single-line, multi-line, number, password), `Select`, `Autocomplete`, `Checkbox`, `RadioGroup` + `Radio`, or `DatePicker` / `DateTimePicker`.
...
- The control is a destructive toggle (archive, delete) → use a [Button](../base/button/usage.md) with a [ConfirmDialog](./dialog/usage.md), not a Checkbox Form Field.
...
- **One of the input surfaces** — [TextField](../base/text-field/usage.md), `Select`, `Autocomplete`, `Checkbox`, `RadioGroup`+`Radio`, `DatePicker` / `DateTimePicker`.
